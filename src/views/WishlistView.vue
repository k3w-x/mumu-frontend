<template>
  <div class="wishlist-page">
    <div class="page-header">
      <h1 class="page-title">Your Wishlist
        <span class="page-count" v-if="wishlist.items.length">({{ wishlist.items.length }} items)</span>
      </h1>
      <p class="page-sub">See the products you liked</p>
    </div>

    <div v-if="wishlist.items.length === 0" class="empty">
      <p>Your wishlist is empty</p>
      <RouterLink to="/catalog" class="empty-link">Browse Catalog →</RouterLink>
    </div>
    ...
    <div v-else class="wishlist-list">
      <div v-for="product in wishlist.items" :key="product.id" class="wishlist-item">
        ...
        <div class="wishlist-item-info">
          <p class="item-brand">{{ product.brand || product.category }}</p>
          <p
            class="item-name"
            @click="$router.push(`/product/${product.id}`)"
          >
          ...
          <p v-if="inStock(product)" class="item-stock">In stock</p>
          <p v-else class="item-stock item-stock--out">Out of stock</p>
        </div>
        ...
        <div class="wishlist-item-actions">
          ...
          <button class="remove-btn" @click="wishlist.remove(product.id)">
            Remove Product 🗑
          </button>
        </div>
      </div>
